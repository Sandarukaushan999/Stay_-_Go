...
export default function NearbyRidersList({ pickup }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4">
      ...
      <p className="mt-1 text-sm text-slate-400">Sorted by OSRM ETA to your pickup point (same university/campus only).</p>
          ...
          {rows.map((r) => (
            <div key={r.id} className="rounded-xl border border-slate-800 bg-slate-950 p-3">
              ...
              <div className="mt-1 text-xs text-slate-400">
                {r.vehicleType ?? 'vehicle'} {r.vehicleNumber ? `• ${r.vehicleNumber}` : ''} • seats {r.seatCount ?? 0} • rating{' '}
                {r.rating ?? 0}
              </div>
            </div>
          ))}
