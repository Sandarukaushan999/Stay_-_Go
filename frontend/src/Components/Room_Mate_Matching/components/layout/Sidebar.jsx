...
const adminLinks = [
    { to: '/roommate/admin/issues', label: 'Issue Management', icon: <FaClipboardList /> },
    { to: '/roommate/admin/rooms', label: 'Room Management', icon: <FaBuilding /> },
];
...
export default function Sidebar() {
    ...
    const getStepStatus = (to) => {
        if (to === '/roommate/dashboard') return 'available';
        if (to === '/roommate/profile') return 'available';
        if (to === '/roommate/preferences') return isProfileComplete ? 'available' : 'locked';
        if (['/roommate/matches', '/roommate/requests', '/roommate/final-result'].includes(to)) {
            return isRoomPrefComplete ? 'available' : 'locked';
        }
        return 'available';
    };
