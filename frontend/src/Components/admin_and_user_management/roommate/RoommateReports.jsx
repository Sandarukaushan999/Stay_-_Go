...
const getIssueImageUrl = (rawUrl) => {
  ...
  if (/^https?:\/\//i.test(normalized)) {
    try {
      ...
      // Fix old broken records saved with frontend origin (5173)
  ...
  return `${backendBase}/${normalized.replace(/^\/+/, "")}`;
};
...
export default function RoommateReports() {
  ...
  const handleForceUnmatch = async (matchId) => {
    if (
      !window.confirm(
        "WARNING: Are you sure you want to forcibly UNMATCH these users? This action is irreversible.",
      )
    )
      return;
    try {
  ...
  const handleAddAdminComment = async (issueId) => {
    ...
    try {
      await api.patch(`/roommate/issues/${issueId}/comment`, {
  ...
  const getStatusUI = (status) => {
    if (status === "RESOLVED")
      return (
        <span className="inline-flex items-center gap-1.5 text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded-lg text-xs font-medium">
          <CheckCircle2 className="w-3.5 h-3.5" /> Resolved
        </span>
      );
    if (status === "IN_PROGRESS")
      return (
        <span className="inline-flex items-center gap-1.5 text-amber-400 bg-amber-400/10 px-2.5 py-1 rounded-lg text-xs font-medium">
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">
              Reports & Match Handling
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              Review roommate pairings, pending requests, and manage issue
              reports.
            </p>
          </div>
                      ...
                      requests.map((r) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            ...
                            {r.status === "REJECTED" && (
                              <span className="inline-flex items-center gap-1.5 text-rose-400 bg-rose-400/10 px-2.5 py-1 rounded-lg text-xs font-medium">
                    ...
                    <div className="flex flex-col lg:flex-row justify-between gap-6">
                      <div className="flex-1 space-y-3">
                        ...
                        {issue.imageUrl && (
                          <div className="mt-3">
                            <a
                              href={getIssueImageUrl(issue.imageUrl)}
                              target="_blank"
                              rel="noreferrer"
                              className="text-xs text-blue-400 hover:text-blue-300 hover:underline inline-flex items-center gap-1 bg-blue-500/10 px-3 py-1.5 rounded-lg border border-blue-500/20"
                            >
                              View Attached Image
                            </a>
                          </div>
                      ...
                      <div className="lg:w-48 flex flex-col gap-2 shrink-0">
                        ...
                        {issue.status !== "IN_PROGRESS" &&
                          issue.status !== "RESOLVED" && (
                            <button
                              ...
                              className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white text-xs font-medium rounded-lg transition-colors text-left border border-slate-700 flex items-center gap-2"
                            >
                        ...
                        {issue.status !== "RESOLVED" && (
                          <button
                            onClick={() =>
                              handleUpdateIssueStatus(issue._id, "RESOLVED")
                            }
                            className="px-3 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 text-xs font-medium rounded-lg transition-colors text-left border border-emerald-500/20 flex items-center gap-2"
                          >
                        ...
                        <button
                          onClick={() => handleAddAdminComment(issue._id)}
                          className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium rounded-lg transition-colors text-left border border-slate-700 mt-1 flex items-center gap-2"
                        >
                          <MessageSquare className="w-3.5 h-3.5" />{" "}
                          {issue.adminComment ? "Edit Comment" : "Add Comment"}
                        </button>
                      </div>
                    </div>
                  </div>
