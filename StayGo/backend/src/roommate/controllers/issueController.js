...
export const createIssue = async (req, res, next) => {
  try {
    const { category, title, description, priority, roomNumber, additionalNotes } = req.body
    ...
    let imageUrl = null
    if (req.file) {
      imageUrl = `${env.CLIENT_URL}/uploads/issues/${req.file.filename}`
    }
...
export const getAllIssues = async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      data: { issues, total, page: Number(page), pages: Math.ceil(total / Number(limit)) }
    })
  } catch (err) {
    next(err)
  }
